...
class LabelerTest(unittest.TestCase):
  ...
  def test_dataset_property(self):
    self.assertEqual(
        self._ds_loader,
        self._ds_labeler._dataset_loader,
        "DatasetLoader instance should be the same for DatasetLabeler "
    )

  def test_images_equal_json_files(self):
    self.assertEqual(
        len(self.ds_images),
        len(self.ds_json_files),
        "Number of images and json files should be equal.")

  def test_multi_hand_landmarks(self):
    for json_file in self.ds_json_files:
      if not "both" in json_file.name:
        continue
